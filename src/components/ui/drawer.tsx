...
const Drawer = memo(({ shouldScaleBackground = true, ...props }: any) => {
  return useMemo(
    () => (
...
const DrawerOverlay = memo(({ className, ...props }: any) => {
  return <DrawerPrimitive.Overlay className={cn('fixed inset-0 z-50 bg-black/80', className)} {...props} />
})
DrawerOverlay.displayName = 'DrawerOverlay'

const DrawerContent = memo(({ className, children, ...props }: any) => {
  return useMemo(
    () => (
      <DrawerPortal>
        <DrawerOverlay />
...
const DrawerTitle = memo(({ className, ...props }: any) => {
  return (
...
const DrawerDescription = memo(({ className, ...props }: any) => {
