...
export const SubmitBookmarkForm = memo(({ className, setFormOpen, bookmarks, currentBookmark }: any) => {
  ...
  const form = useForm(memoizedFormOptions as any)
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={cn('flex flex-col gap-6', className)}>
        ...
        <Button type="submit" className="w-full" disabled={isSubmitting || (errors as any)?.api?.limitError || !isValid}>
          {hasErrors ? (
            'Submit'
          ) : (
